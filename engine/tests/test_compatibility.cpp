...
TEST(CompatibilityService, ExactMatch) {
    EXPECT_TRUE(efl::CompatibilityService::isCompatible("1.1.0", "1.1.0"));
}

TEST(CompatibilityService, NewerEflSatisfiesOlderRequirement) {
    EXPECT_TRUE(efl::CompatibilityService::isCompatible("1.2.0", "1.1.0"));
}

TEST(CompatibilityService, OlderEflFailsNewerRequirement) {
    EXPECT_FALSE(efl::CompatibilityService::isCompatible("1.0.0", "1.1.0"));
}

TEST(CompatibilityService, MajorVersionMismatch) {
    EXPECT_FALSE(efl::CompatibilityService::isCompatible("0.9.0", "1.1.0"));
}
