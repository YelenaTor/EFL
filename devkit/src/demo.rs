...
pub fn start_demo() -> mpsc::Receiver<IpcMessage> {
    ...
    thread::spawn(move || {
        let send =
            |tx: &mpsc::Sender<IpcMessage>, msg: IpcMessage| -> bool { tx.send(msg).is_ok() };
        ...
        if !send(
            ...
            msg(
                "phase.transition",
                serde_json::json!({ "phase": "diagnostics" }),
            ),
        ) {
            return;
        }
        ...
        let diagnostics = [
            (
                ...
                "error",
                "MANIFEST",
                "Missing required field 'version' in example-mod.efl",
            ),
            (
                "HOOK-W003",
                "warning",
                "HOOK",
                "Hook target gml_Script_hoe_node may have changed in game update 0.14",
            ),
            (
                "AREA-H002",
                "hazard",
                "AREA",
                "Area 'crystal_cave' references room r_mine_04 which may conflict with base game",
            ),
            (
                "MANIFEST-W001",
                "warning",
                "MANIFEST",
                "Optional field 'author_url' is empty",
            ),
            (
                "NPC-E002",
                "error",
                "NPC",
                "NPC 'flora_spirit' references undefined dialogue tree 'flora_intro'",
            ),
            (
                "SAVE-W001",
                "warning",
                "SAVE",
                "Save namespace 'EFL/example-mod' exceeds recommended key count (>50)",
            ),
            (
                "TRIGGER-H001",
                "hazard",
                "TRIGGER",
                "Trigger 'unlock_cave' uses allOf with 5+ conditions — consider splitting",
            ),
            (
                "BOOT-W001",
                "warning",
                "BOOT",
                "Hook registry fallback mode active — some targets may be unavailable",
            ),
            (
                "RESOURCE-E001",
                "error",
                "RESOURCE",
                "Resource node 'mythril_ore' sprite sheet not found at sprites/mythril.png",
            ),
            (
                "QUEST-W002",
                "warning",
                "QUEST",
                "Quest 'find_crystals' has no fail condition defined",
            ),
        ];
        ...
        if !send(
            ...
            msg(
                "momi.mod.status",
                serde_json::json!({
                    "modId": "momi.flora.base",
                    "name": "Flora Base Data",
                    "version": "1.4.0",
                    "status": "loaded"
                }),
            ),
        ...
        if !send(
            ...
            msg(
                "compat.relationship",
                serde_json::json!({
                    "packId": "dev.yoru.crystal-caves",
                    "momiModId": "momi.deepdungeon",
        ...
        if !send(
            ...
            msg(
                "compat.relationship",
                serde_json::json!({
                    "packId": "dev.yoru.flora-expansion",
                    "momiModId": "momi.flora.base",
        ...
        for cycle in 0..60 {
            ...
            // Item grant from story event (quest reward)
            if cycle == 35 {
                if !send(
                    &tx,
                    msg(
